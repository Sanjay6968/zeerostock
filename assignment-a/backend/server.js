...
// Serve the frontend static files so visiting http://localhost:3001/ loads the UI
const path = require('path');
app.use(express.static(path.join(__dirname, '../frontend')));

// In-memory array of 10-15 records
const inventory = [
...
app.get('/search', (req, res) => {
  ...
  if (q) {
    ...
    results = results.filter(item => searchRegex.test(item.name));
  }
...
app.listen(PORT, () => {
  console.log(`Assignment A Backend running on http://localhost:${PORT}`);
});
